...
def get_connection():
    return mysql.connector.connect(
        host="localhost",
        user="root",
        password="Monkeybeatszilla2", #password for mysql database, change it to your own passowrd
...
def get_user_by_id(user_id):
    query = "SELECT library_id, username, email, first_name, last_name, role FROM person WHERE library_id = %s"
    return query_db(query, (user_id,), fetchone=True)

def update_user(user_id, username, email, first_name, last_name, role):
    query = """
        UPDATE person
        SET username = %s, email = %s, first_name = %s, last_name = %s, role = %s
        WHERE library_id = %s
    """
    return query_db(query, (username, email, first_name, last_name, role, user_id))


def delete_user_by_id(user_id):
    """Safely drops a user profile row while managing data integrity blocks."""
    conn = get_connection()
    cursor = conn.cursor()
    try:
